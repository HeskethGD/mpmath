...
factorization of ``Im(tau)`` is used to enumerate the required lattice
points, with the truncation radius selected from an incomplete-gamma tail
bound. Half-integer upper gamma factors use an elementary exponential upper
bound followed by positive-term recurrence, avoiding repeated high-precision
``erfc`` evaluations while keeping the truncation estimate conservative.
